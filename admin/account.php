...
<?php
	...
	<div class="widget-body">';
        ...
        echo '<div class="widget-title"><i class="icon-user"></i>&nbsp;Tài khoản</div>
	<div class="widget-body">';
		$sql = "SELECT * FROM `admin` where `user` = '$account'";
		...
		if(isset($_POST['update'])) {
			...
			$sql = "UPDATE `admin` SET `fullname` = '$fullname', `sex` = '$sex', `phone` = '$phone', `address` = '$address' WHERE `user` = '$account';";
			$result = @mysql_query($sql);
			if(isset($result)) {
		...
		if(isset($_POST['submit'])) {
			...
			elseif($renewpassword != $newpassword)
				...
				$sql = "UPDATE `admin` SET `password` = '$pw' WHERE `user` = '$account';";
				$result = @mysql_query($sql);
				if(isset($result))
